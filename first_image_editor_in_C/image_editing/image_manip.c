	...
	if(IMAGE_ONE_MORE_ROWS && IMAGE_TWO_MORE_COLS){
		...
		for(int i = 0; i<in1.rows; i++){
			for(int j = 0; j<in1.cols ; j++){
				blend.data[next_col+j] = in1.data[k];
				k++;
			}
			next_col += max_col;
		}
	...
	if(IMAGE_ONE_MORE_COLS && IMAGE_TWO_MORE_ROWS){
		...
		for(int i = 0; i<in2.rows; i++){
			for(int j = 0; j<in2.cols ; j++){
				blend.data[next_col+j] = in2.data[k];
				k++;
			}
			next_col += max_col;
		}
		...
		for(int i = 0; i<min_row; i++){
			for(int j = 0; j<min_col; j++){
				blended_rpix_in1 = in2.data[k].r * alpha;
				blended_gpix_in1 = in2.data[k].g * alpha;
				blended_bpix_in1 = in2.data[k].b * alpha;
				blended_rpix_in2 = in1.data[j+next_row].r * (1-alpha);
				blended_gpix_in2 = in1.data[j+next_row].g * (1-alpha);
				blended_bpix_in2 = in1.data[j+next_row].b * (1-alpha);
				blend.data[j+next_row].r = blended_rpix_in1 + blended_rpix_in2;
				blend.data[j+next_row].g = blended_gpix_in1 + blended_gpix_in2;
				blend.data[j+next_row].b = blended_bpix_in1 + blended_bpix_in2;
				k++;
...
Image blur( const Image in , double sigma ){
	...
	for(int i =0; i<N; i++){
    	...
    	dy++;
    	dx = position;
	} // the Gaussian matrix has been made

    int rows = in.rows;
    int cols = in.cols;

	Pixel ** twoDimInput = (Pixel **)calloc(rows, sizeof(Pixel*));
	Pixel ** twoDimResult = (Pixel **)calloc(rows, sizeof(Pixel*));
	

	for (int r = 0; r < rows; r++) {
		twoDimInput[r] = (Pixel *)malloc(cols * sizeof(Pixel));
		twoDimResult[r] = (Pixel *)malloc(cols * sizeof(Pixel));

	} // now we have a 2D array of all the image pixels, and a similar 2D array ready to receive results, but values are still
	 //uninitialized.

	int numZeroRed = 0;
	for (int r = 0; r < rows; r++) {
		for (int c = 0; c < cols; c++) {
			twoDimInput[r][c].r = in.data[r * cols + c].r;
			twoDimInput[r][c].g = in.data[r * cols + c].g;
			twoDimInput[r][c].b = in.data[r * cols + c].b;

			if (twoDimInput[r][c].r == 0) {
				numZeroRed++;
			}
		}
	}
	...
	for (int r = 0; r < rows; r++) {
		for (int c = 0; c < cols; c++) {
			...
			double greenSum = 0.0;
			double blueSum = 0.0;
			Pixel currentPix = twoDimInput[r][c];
			if (currentPix.r == 0) {
				zeroRedIn2ndLoop++;
			}
			for (int gR = 0; gR < N; gR++) {
				...
				for (int gC = 0; gC < N; gC++) {
					...
					if (!(curRow < 0 || curRow > rows - 1 || curCol < 0 || curCol > cols - 1)) {
						wasValidPixel = true;
						Pixel curpix = twoDimInput[curRow][curCol];

						redSum += (double)curpix.r * kernel[gR][gC];
						greenSum += (double)curpix.g * kernel[gR][gC];
						blueSum += (double)curpix.b * kernel[gR][gC];

						if (curpix.r == 0) {
							numRed2++;
						}

						matrixSum += kernel[gR][gC];
			...
			twoDimResult[r][c].r = (unsigned char) (redSum / matrixSum);
			twoDimResult[r][c].g = (unsigned char) (greenSum / matrixSum);
			twoDimResult[r][c].b = (unsigned char) (blueSum / matrixSum);
			
		}
	}

	// copy over the 2D array into the data of our final blurImage
	Image blurImage;
	blurImage = make_image(rows, cols);


	for (int r = 0; r < rows; r++) {
		for (int c = 0; c < cols; c++) {
			blurImage.data[r*cols + c] = twoDimResult[r][c];
		}
	}

	for (int r = 0; r < rows; r++) { // freeing 2D arrays 
		free(twoDimInput[r]);
		free(twoDimResult[r]);
	}
	free(twoDimInput);
