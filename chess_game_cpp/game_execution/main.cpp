        ...
        // exit the program with return code -1 if an exception is caught here
			...
			case 'M': case 'm': {
				...
				// And make the move
          		...
          		// Try catch blocks to recover from illegral moves
					try{
						game.make_move(std::make_pair(argument[0], argument[1]),
							std::make_pair(argument[2], argument[3]));
					} catch(Chess::StartPositionNotOnBoardException &e){
						std::cerr << "Could not make move: " << e.what() << std::endl;
					} catch(Chess::EndPositionNotOnBoardException &e) {
						std::cerr << "Could not make move: " << e.what() << std::endl;
					} catch(Chess::NoPieceAtStartException &e){
						std::cerr << "Could not make move: " << e.what() << std::endl;
					} catch(Chess::PieceColorTurnDontMatchException &e) {
						std::cerr << "Could not make move: " << e.what() << std::endl;
					} catch(Chess::IllegalMoveShapeException &e){
						std::cerr << "Could not make move: " << e.what() << std::endl;
					} catch(Chess::CaptureOwnPieceException &e) {
						std::cerr << "Could not make move: " << e.what() << std::endl;
					} catch(Chess::IllegalCaptureShapeException &e){
						std::cerr << "Could not make move: " << e.what() << std::endl;
					} catch(Chess::PathNotClearException &e) {
						std::cerr << "Could not make move: " << e.what() << std::endl;
					} catch(Chess::MoveExposesCheckException &e) {
						std::cerr << "Could not make move: " << e.what() << std::endl;
					}
